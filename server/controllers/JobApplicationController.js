...
exports.getFilterJobApplicationByEmployeeId = async (req, res) => {
    const employeeId = req.params.employeeId;
    const { jobTitle, page = 0, pageSize = 2 } = req.body; // Includes page and pageSize

    try {
        let filter = { employeeId };

        // If a jobTitle filter is provided, find matching job IDs
        if (jobTitle) {
        ...
        // Fetch the total number of applications first
        const totalApplications = await JobApplication.countDocuments(filter);
        const totalPages = Math.ceil(totalApplications / pageSize);

        // Fetch the job applications with the applied filters and sort by applicationDate
        const jobApplications = await JobApplication.find(filter)
            .populate('jobId')
            .sort({ applicationDate: -1 }) // Sort by applicationDate in descending order
            .skip(skip)
            .limit(pageSize);
...
exports.getFilterJobApplicationsByCompanyId = async (req, res) => {
    const companyId = req.params.companyId;
    const { jobTitle, page = 0, pageSize = 2 } = req.body; // Thêm page và pageSize
...
exports.getFilterJobApplicationsByCompanyIdStatus = async (req, res) => {
    const companyId = req.params.companyId;
    const { jobStatus, page = 0, pageSize = 2 } = req.body; // Add page and pageSize

    try {
        // Find job IDs for the given company ID
        ...
        // Count total job applications matching the query
